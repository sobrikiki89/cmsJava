...
public class ClaimByLossTypeParamHandler extends ReportParamHandler {
	...
	public void setupLookup() {
		...
		LossTypeService lossTypeService = context.getBean(LossTypeService.class);
		lookupMap.put("lossTypes",lossTypeService.getActiveLossTypes());
	}
	...
	public String formatParams() {
		...
		if (paramMap.get("fromLossDate") != null && !"".equals(paramMap.get("fromLossDate"))  && paramMap.get("fromLossDate") instanceof String) {
			...
			paramMap.put("dispFromLossDate", sdfD.format(fromLossDate));
		} else {
			paramMap.put("fromLossDate", null);
			paramMap.put("dispFromLossDate", null);
		}

		paramStr.append(", ");
		paramStr.append("toLossDate=");
		if (paramMap.get("toLossDate") != null && !"".equals(paramMap.get("toLossDate")) 
				&& paramMap.get("toLossDate") instanceof String) {
			...
			paramMap.put("dispToLossDate", sdfD.format(toLossDate));
		} else {
			paramMap.put("toLossDate", null);
		...
		paramStr.append(", ");
		paramStr.append("lossTypeCode=");
		if (paramMap.get("lossTypeCode") != null) {
			LossTypeService lossTypeService = context.getBean(LossTypeService.class);

			StringBuilder dispLossType = new StringBuilder();
			List<String> lossTypeCodeList = new ArrayList<String>();
			if (paramMap.get("lossTypeCode").getClass().isArray()) {
				LOGGER.info("Multiple Loss Type selected");
				String[] lstLossType = (String[]) paramMap.get("lossTypeCode");
				for (String lossTypeCode : lstLossType) {
					if (lossTypeCodeList.size() > 0) {
						paramStr.append(",");
						dispLossType.append(", ");
					}
					...
					if (lossType != null) {
						dispLossType.append(lossType.getName());
					}

					lossTypeCodeList.add(lossTypeCode);
					paramStr.append(lossTypeCode);
					LOGGER.info("Loss Type Code=" + lossTypeCode);
				}
			} else if (paramMap.get("lossTypeCode") instanceof String) {
				LOGGER.info("Single Loss Type selected");
				String lossTypeCode = (String) paramMap.get("lossTypeCode");
				LossType lossType = lossTypeService.getLossType(lossTypeCode);
				if (lossType != null) {
					dispLossType.append(lossType.getName());
				}
			...
			if (lossTypeCodeList.size() > 0) {
				paramMap.put("lossTypeCode", lossTypeCodeList.toArray());
			} else {
				paramMap.put("lossTypeCode", null);
			}
			paramMap.put("dispLossType", dispLossType.toString());
		}
