...
public class OverallClaimStatParamHandler extends ReportParamHandler {
	...
	public String formatParams() {
		...
		paramStr.append("fromNotifyDate=");
		if (paramMap.get("fromNotifyDate") != null && paramMap.get("fromNotifyDate") instanceof String) {
			Timestamp fromNotifyDate = null;
			String strFromNotifyDate = (String) paramMap.get("fromNotifyDate");
			try {
				DateTimeFormatter formatter = DateTimeFormat.forPattern("dd-MMM-yyyy");
				DateTime dt = formatter.parseDateTime(strFromNotifyDate);
				dt = dt.withTimeAtStartOfDay();
				fromNotifyDate = new Timestamp(dt.getMillis());
				paramStr.append(fromNotifyDate);
				LOGGER.info("from Notify Date=" + fromNotifyDate);
			} catch (Exception ignore) {
				LOGGER.warn("Error in formating fromNotifyDate [" + strFromNotifyDate + "]");
			}

			paramMap.put("fromNotifyDate", fromNotifyDate);
			paramMap.put("dispFromNotifyDate", fromNotifyDate);
		}

		paramStr.append(", ");
		paramStr.append("toNotifyDate=");
		if (paramMap.get("toNotifyDate") != null && paramMap.get("toNotifyDate") instanceof String) {
			Timestamp toNotifyDate = null;
			String strToNotifyDate = (String) paramMap.get("toNotifyDate");
			// 01-Jan-2016
			try {
				DateTimeFormatter formatter = DateTimeFormat.forPattern("dd-MMM-yyyy");
				DateTime dt = formatter.parseDateTime(strToNotifyDate);
				dt = dt.plusDays(1).withTimeAtStartOfDay().minusMillis(1);
				toNotifyDate = new Timestamp(dt.getMillis());
				paramStr.append(toNotifyDate);
				LOGGER.info("to Notify Date=" + toNotifyDate);
			} catch (Exception ignore) {
				LOGGER.warn("Error in formating toNotifyDate [" + strToNotifyDate + "]");
			}

			paramMap.put("toNotifyDate", toNotifyDate);
			paramMap.put("dispToNotifyDate", toNotifyDate);
		}
