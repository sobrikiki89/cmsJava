...
<spring:url var="postUrl" value="/secured/setup/adjuster" />
<spring:url var="gridUrl" value="/secured/setup/adjuster/grid" />

<script type="text/javascript">

	$(document).ready(function() {
		$('#adjusterForm').on('keyup keypress', function(e) {
	...
	function edit(cellvalue, options, rowObject) {
		return '<a href=\"<spring:url value="/secured/setup/adjuster/edit/"/>${url_param_prefix}/' + rowObject.id + '\"><spring:message code="label.edit"/></a>';
	}
...
<div class="class-container">
	<form:form commandName="adjusterForm" action="${postUrl}">
	<div class="row">
	    <div class="col-sm-1"></div>
	  	<div class="panel-group col-sm-10">
		    <div class="panel panel-primary">
		    	<div class="panel-heading"><spring:message code="setup.adjuster.listing" /></div>
				<form:errors element="div" cssClass="errorblock" path="*" />
				
				<div class="panel-body">
					<datatables:table id="adjusterListTableId" cssClass="table table-striped" url="${gridUrl}" 
						...
						<datatables:column renderFunction="checkbox" property="id" searchable="false" sortable="false" cssStyle="padding:10px 10px; width=5%">
						  <datatables:columnHead>
						    <input type="checkbox" onclick="$('#adjusterListTableId').find('input:checkbox').prop('checked', this.checked);" />
						  </datatables:columnHead>
						</datatables:column>
						<datatables:column titleKey="setup.adjuster.firmName" property="firmName" cssStyle="padding:10px 10px; width=45%" sortInitOrder="0" sortInitDirection="asc"/>
