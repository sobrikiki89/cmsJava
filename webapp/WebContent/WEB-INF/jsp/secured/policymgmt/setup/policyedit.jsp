...
<spring:url var="saveUrl" value="/secured/policymgmt/setup/edit" />
...
<script type="text/javascript">
	...
	function back() {
		$("#policySetupForm").attr("action", "${backUrl}").submit();
	}	
	...
	$(document).ready(function () {
		...
		$("input#interesInsuredAddRow").click(function() {
			...
			var html = "<tr><td><input id='policy.interestInsuredList" + idx + ".order' name='policy.interestInsuredList[" + idx + "].order' readonly='readonly' type='text' class='form-control input-sm' value='" + (idx + 1) + "'/>" + 
			"<input id='policy.interestInsuredList" + idx + ".id' name='policy.interestInsuredList[" + idx + "].id' type='hidden'/></td>" +
		...
		$("input#excessDeductibleAddRow").click(function() {
			...
			var html = "<tr><td><input id='policy.excessDeductibleList" + idx + ".order' name='policy.excessDeductibleList[" + idx + "].order' readonly='readonly' type='text' class='form-control input-sm' value='" + (idx + 1) + "'/>" +
			"<input id='policy.excessDeductibleList" + idx + ".id' name='policy.excessDeductibleList[" + idx + "].id' type='hidden'/></td>" +
			...
			"<td><a id='dExcessDeductible" + idx + "' href='#' onclick='deleteExcessDeductibleRow(this)'><span class='glyphicon glyphicon-trash'></span></a></td</tr>";			
	    ...
	    <div class="col-sm-1"></div>
	  	<div class="panel-group col-sm-12">
		    <div class="panel panel-primary">
		    	<div class="panel-heading"><spring:message code="policymgmt.setup.editPolicy" /></div>
					...
					<div class="tab-content">
						<div id="tab-policy" class="tab-pane fade in active panel-body">
							<div class="form-group row">
		  						...
		  						<span class="col-sm-3">
									<c:choose>
										<c:when test="${policySetupForm.allowedEditPolicyNo}">
				   							<form:input path="policy.policyNo" class="form-control input-sm" id="policyNo"/>
										</c:when>
										<c:otherwise>
				   							<form:input path="policy.policyNo" class="form-control input-sm" id="policyNo" readonly="true"/>
										</c:otherwise>
									</c:choose>
		   						</span>
		  						...
		  						<span class="col-sm-3">
		  							<form:hidden path="policy.insuranceClass.code" />
							...
							<div class="form-group row">
								...
								<span class="col-sm-9">
		  							<form:hidden path="policy.insurer.code" />								
		  							<form:select path="policy.insurer.code" class="form-control input-sm" id="insurerName" disabled="true">
										<form:option label="Select" value="" />
									...
									<table id="interestInsuredTable" class="table table-bordered table-hover">
										...
										<tbody>
											<c:forEach var="interestInsuredItem" varStatus="i" begin="0" items="${policySetupForm.policy.interestInsuredList}">
												<tr>
													<td>
														<form:input path="policy.interestInsuredList[${i.index}].order" class="form-control input-sm" id="policy.interestInsuredList${i.index}.order" readonly="true"/>
														<form:hidden path="policy.interestInsuredList[${i.index}].id" id="policy.interestInsuredList${i.index}.id" />
													</td>
													...
													<td><form:input path="policy.interestInsuredList[${i.index}].sumCovered" class="form-control input-sm" id="policy.interestInsuredList${i.index}.sumCovered"/></td>
									...
									<table id="excessDeductibleTable" class="table table-bordered table-hover">
										...
										<tbody>
											<c:forEach var="excessDeductibleItem" varStatus="i" begin="0" items="${policySetupForm.policy.excessDeductibleList}">
												<tr>
													<td>
														<form:input path="policy.excessDeductibleList[${i.index}].order" class="form-control input-sm" id="policy.excessDeductibleList${i.index}.order" readonly="true"></form:input>
														<form:hidden path="policy.excessDeductibleList[${i.index}].id" id="policy.excessDeductibleList${i.index}.id" />
													</td>
													...
													<td><form:input path="policy.excessDeductibleList[${i.index}].amount" class="form-control input-sm" id="policy.excessDeductibleList${i.index}.amount"></form:input></td>
									...
									<table id="attachmentTable" class="table table-bordered table-hover">
										<thead>
											<tr>
												...
												<th class="text-center col-sm-6"><spring:message code="label.name" /></th>
												<th class="text-center col-sm-1"></th>
												<th class="text-center col-sm-1"></th>
											</tr>
										</thead>
										<tbody>
											...
											<tr>
												<td>
													<label id="filesList${i.index}.order" class="text-center" style="">${i.index+1}</label>
									...
									<table id="endorsementTable" class="table table-bordered table-hover">
										...
										<tbody>
											...
											<tr>
												...
												<c:choose>
													<c:when test="${policySetupForm.allowedDeleteEndorsement}">
								                		<td><a id='dEndorsement${i.index}' href='#' onclick='deleteEndorsementRow(this)'><span class="glyphicon glyphicon-trash"></span></a></td>
													</c:when>
													<c:otherwise>
								                		<td><a id='dEndorsement${i.index}' href='#'><span class="glyphicon glyphicon-minus-sign"></span></a></td>
													</c:otherwise>
												</c:choose>
											</tr>
											</c:forEach>
				...
				<div class="panel-footer" align="right">
					<input name="action" type="submit" class="btn btn-primary" value="<spring:message code="button.submit" />" />
